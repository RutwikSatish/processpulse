...
def fig_process_flow(steps: list, process: str) -> go.Figure:
    """Process flow with alternating label positions to prevent overlap."""
    n = len(steps)
    x = list(range(n))
    ...
    node_sizes  = [22 if s in BOTTLENECK_STEPS else 16 for s in steps]

    # Alternate y positions: even nodes above, odd nodes below the line
    y_nodes = [0.18 if i % 2 == 0 else -0.18 for i in range(n)]
    text_pos = ["top center" if i % 2 == 0 else "bottom center" for i in range(n)]

    fig = go.Figure()

    # Connector lines from node to line
    for i, (xi, yi) in enumerate(zip(x, y_nodes)):
        fig.add_shape(type="line",
            x0=xi, x1=xi, y0=0, y1=yi,
            line=dict(color=C_BORDER, width=1, dash="dot"))
    ...
    # Nodes — one trace per node so textposition can vary
    for i, (xi, yi, step, col, sz, tp) in enumerate(
            zip(x, y_nodes, steps, node_colors, node_sizes, text_pos)):
        is_bn = step in BOTTLENECK_STEPS
        fig.add_trace(go.Scatter(
            x=[xi], y=[yi],
            mode="markers+text",
            text=[step],
            textposition=tp,
            textfont=dict(size=9, color=C_DANGER if is_bn else C_TEXT),
            marker=dict(size=sz, color=col, line=dict(color=C_BG, width=2)),
